/**
 * JournalNevis v5.8.2 - Google Apps Script backend
 * ------------------------------------------------------------
...
function onOpen() {
  ...
  ui.createMenu('JournalNevis v5.8.2')
...
function setupJournalNevis() {
  ...
  SpreadsheetApp.getUi().alert(
    'JournalNevis v5.8.2 setup completed.\\n\\n' +
...
function handleLiveBundle_(ss, p) {
  ...
  // v5.8.2: refresh the dashboard automatically for the account currently selected
  // on Dashboard. This keeps live journaling responsive, but avoids forcing the
  // user to press a manual sync button after each new trade/update.
  try {
    const selectedAccountKey = refreshDashboardAccountSelector_(ss);
    const liveAccountKey = accountKey_(
      p.account && p.account.account_login ? p.account.account_login :
      (p.trade && p.trade.account_login ? p.trade.account_login : ''),
      p.account && p.account.server ? p.account.server :
      (p.trade && p.trade.server ? p.trade.server : '')
    );

    if (!selectedAccountKey || selectedAccountKey === liveAccountKey || accountKeys_(ss).length <= 1) {
      compactAndResequenceTrades_(ss);
      normalizeCashFlows_(ss);
      refreshAnalyticsAndDashboard_(ss);
    }
  } catch (err) {
    SpreadsheetApp.getActive().toast('Live dashboard refresh failed: ' + err.message, 'JournalNevis', 5);
  }

  return jsonResponse_({ ok:true, action:'LIVE_BUNDLE', message:'Live trade bundle stored and dashboard refreshed.' });
}
...
function handleCashFlow_(ss, p) {
  ...
  // JournalNevis v5.8.2: live writes stay fast. Dashboard/normalization is performed once
...
function handleSyncComplete_(ss, p) {
  ...
  // JournalNevis v5.8.2: schema migration belongs to setupJournalNevis(), not every sync.
  ...
  return jsonResponse_({
    ...
    message: 'JournalNevis v5.8.2 Stage 4 completed; rows preserved and dashboard refreshed once.'
  });
}
...
function verifyScreenshotLinkPersistence() {
  ...
  SpreadsheetApp.getUi().alert(
    'JournalNevis v5.8.2 screenshot link check\\n\\n' +
